...
def load_bookcrossing(path_to_data='data/book_crossing/book_ratings.dat', subset_percent_rows=1.0, subset_percent_cols=1.0):
    """
    Loads and prepares the Book-Crossing dataset. Normalizes ratings to [0, 1]. Optionally subselects users/books.
    """
    try:
        df = pd.read_csv(path_to_data, sep='\t', header=0, encoding='latin-1')
        print(df.head())
        df.columns = ['User_ID', 'ISBN', 'Book_Rating']
    except FileNotFoundError:
        print("Dataset not found. Please download Book-Crossing and place BX-Book-Ratings.csv in a 'book-crossing' folder.")
        print("Download from: https://www.kaggle.com/datasets/ruchi798/bookcrossing-dataset")
        return None, None

    # Consider only ratings > 0 (as per Book-Crossing convention)
    ...
    # Subset books
    if subset_percent_cols < 1.0:
        book_sample = np.random.choice(df['ISBN'].unique(),
                                       int(len(df['ISBN'].unique()) * subset_percent_cols),
                                       replace=False)
        df = df[df['ISBN'].isin(book_sample)]

    # Normalize ratings to [0, 1]
    min_rating = df['Book_Rating'].min()
    max_rating = df['Book_Rating'].max()
    if max_rating > min_rating:
        df['Book_Rating'] = (df['Book_Rating'] - min_rating) / (max_rating - min_rating)
    else:
        df['Book_Rating'] = 0.0

    user_map = {uid: i for i, uid in enumerate(df['User_ID'].unique())}
    book_map = {isbn: i for i, isbn in enumerate(df['ISBN'].unique())}

    df['user_idx'] = df['User_ID'].map(user_map)
    df['book_idx'] = df['ISBN'].map(book_map)

    num_users, num_books = len(user_map), len(book_map)

    train_df, test_df = train_test_split(df, test_size=0.4, random_state=42)

    train_matrix = csc_matrix((train_df['Book_Rating'], (train_df['user_idx'], train_df['book_idx'])), shape=(num_users, num_books))
    test_matrix = csc_matrix((test_df['Book_Rating'], (test_df['user_idx'], test_df['book_idx'])), shape=(num_users, num_books))
